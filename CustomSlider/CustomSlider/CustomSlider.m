...
/** 滑块的大小 */
#define kSliderBtnWH  16.0
/** 间距 */
//#define kProgressMargin 0
/** 进度的宽度 */
#define kProgressW 5.0
/** 进度的高度 */
#define kProgressH 150
...
- (void)layoutSubviews {
    ...
    self.bgProgressView.centerX     = self.width * 0.5;
//    self.bufferProgressView.centerY = self.height * 0.5;
    self.sliderProgressView.centerX = self.width * 0.5;
    self.bgProgressView.height       = self.height;
    self.sliderBtn.centerX          = self.width * 0.5;
}
- (void)addSubViews {
...
//    [self addSubview:self.bufferProgressView];
    ...
    self.bgProgressView.frame = CGRectMake(0, 0, kProgressW, self.height);
...
- (void)setValue:(float)value {
    ...
    CGFloat finishValue  = self.bgProgressView.frame.size.height * value;
    self.sliderProgressView.height = finishValue;
    
    CGFloat buttonY = (self.height - self.sliderBtn.height) * value;
    self.sliderBtn.top = buttonY;
...
- (void)sliderBtnDragMoving:(UIButton *)btn event:(UIEvent *)event {
    ...
    float value = (point.y - btn.height * 0.5) / (self.height - btn.height);
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    // 获取进度
    float value = (point.y - self.bgProgressView.top) * 1.0 / self.bgProgressView.height;
...
- (UIImageView *)bgProgressView {
    if (!_bgProgressView) {
        ...
        _bgProgressView.layer.cornerRadius = 2.0;
...
//        _bufferProgressView.layer.cornerRadius = 2.0;
...
- (UIImageView *)sliderProgressView {
    if (!_sliderProgressView) {
        ...
        _sliderProgressView.layer.cornerRadius = 2.0;
